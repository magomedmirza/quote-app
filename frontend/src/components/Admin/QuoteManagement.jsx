...
import { Quote, Edit, Trash2, PlusCircle, Save } from "lucide-react";
...
const getUserIdFromLocalStorage = () => {
  return localStorage.getItem("userId") || "1"; // Default fallback
};
...
function QuoteManagement() {
  ...
  const handleNewQuoteSubmit = useCallback(
    ...
    [newQuote, fetchQuotes, handleCloseAddModal]
  );
  ...
  const handleEditQuoteSubmit = useCallback(
    ...
    [fetchQuotes, handleCloseEditModal]
  );
  ...
  const handleDeleteQuoteConfirm = useCallback(
    ...
    [fetchQuotes, handleCloseDeleteModal]
  );
  ...
  return (
    <div className="p-8 bg-white rounded-lg shadow-xl relative">
      ...
      <p className="text-gray-600 mb-6">Kelola Quote</p>
      {isLoading && (
        ...
        <div className="overflow-x-auto mt-6">
          {quotes.length === 0 ? (
            <div className="p-4 bg-yellow-100 border border-yellow-400 text-yellow-700 rounded-lg">
              Tidak ada quote ditemukan. Klik "Tambah Quote" untuk memulai.
            </div>
          ) : (
