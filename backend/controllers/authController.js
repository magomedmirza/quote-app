...
const { verifyPw } = require("../config/bcrypt");
const {
  generateTokens,
  verifyRefreshToken,
  deleteRefreshToken,
...
const login = async (req, res) => {
  try {
    ...
    // Hapus semua refresh token lama user ini - GUNAKAN SERVICE
...
const refreshToken = async (req, res) => {
  ...
  try {
    ...
    // 2. Cek Token di Database menggunakan service
    const tokenExists = await isRefreshTokenExists(oldRefreshToken);
    if (!tokenExists) {
